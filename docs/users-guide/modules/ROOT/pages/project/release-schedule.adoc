= MixQL Release Schedule
:navtitle: Release Schedule
:page-aliases: version-and-lifecycle-policies.adoc, project/roadmap.adoc
:table-caption!:

Each major release line of the MixQL Core components is typically supported for one year after it enters general availability (GA).
...
General Availability (GA):: A release line, such as MixQL 2.x, enters general availability on the date the initial, final major version (e.g., MixQL 2.0.0) of the software is released and available for download.
...
MixQL follows the semantic versioning rules.
Each MixQL component release is versioned *major.minor.patch*.
